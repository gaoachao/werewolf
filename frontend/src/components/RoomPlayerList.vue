<template>
  <div class="room-player-list">
    <div v-for="item in playerList" :key="item.index" class="player">
      <div v-if="item.name !== undefined  " class="box" :style="{ cursor: isActing ? 'pointer' : 'inherit' }" :class="{
        isDead: !item.isAlive,
...
">
        ...
        <img v-show="item.isSheriff" alt="警长" :src="`/assets/sheriff${theme}.svg`" class="sherrif" />
        <img class="dead" :class="{ isDying: item.isDying }" v-show="!item.isAlive" alt="骷髅"
...
export default defineComponent({
  props: {
    playerList: {
      // 提供一个比Object更具体的类型  这里的any是用来解决item不含有某属性的报错
      // PropType<(PublicPlayerDef | { index: number})[]> 这样写还是会报错
      type: Object as PropType<(PublicPlayerDef | any)[]>,
      required: true,
    }
  },
  setup(props) {
    return { theme, setTarget, isActing, target}
  }
})
</script>
